...
class CreditsControllerTest < ActionController::TestCase
  ...
  def test_add_credit_without_package
    assert_difference -> { Credit.count }, 0 do
      post :create, credit: { some_param: 1 }
    end
  end

  def test_add_new_credit
    assert_difference -> { Credit.count }, +1 do
      post :create, credit: { credits_package_id: CreditsPackage.last.id }
      assert_redirected_to 'http://paypal.com?token'
    end
  end

  def test_success_complete_purchase
    credit = @user.credits.create(credits_package_id: CreditsPackage.last.id)
